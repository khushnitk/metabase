    ...
    <div className={cx(S.sidebar, className)} style={style}>
        <ul>
            {sections.map(section =>
                <QuestionSidebarItem key={section.id} href={"/metabase/questions/" + section.id} {...section} />
            )}
            <QuestionSidebarSectionTitle name="Labels" href="/metabase/questions/edit/labels" />
        </ul>
        <LoadingAndErrorWrapper loading={labelsLoading} error={labelsError} noBackground noWrapper>
        { () => labels.length > 0 ? // eslint-disable-line
            <ul>
            { labels.map(label =>
                <QuestionSidebarItem key={label.id} href={"/metabase/questions/label/"+label.slug} {...label} />
            )}
            </ul>
        :
        ...
        <ul>
            ...
            <QuestionSidebarItem name="Archive" href="/metabase/questions/archived" icon="archive" />
        </ul>
    </div>
